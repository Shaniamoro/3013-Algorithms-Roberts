...
public:
    ...
    double distanceTotal = 0.0;  //miles total
    int edge_count = 0;
    ...
    /* Creates a Spanning Tree for each starting state
     * Params: string filter - state
     *          int sizeOfDegree: Max number of Edges
     *
     */

    // find the three closest vertices and create edges between them.
    void createSpanningTree(string statekey, int sizeOfDegree=3)
    {
        ...
        for (i = vertexList.begin(); i != vertexList.end(); i++) {
            ...
            for (int count = 0; count < sizeOfDegree; ++count) {
                ...
                if (isFound) {
                    addEdge((*i)->ID, minID, minDistance, true);
                    edge_count++;
